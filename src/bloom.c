...
struct BloomFilter *bloom_create(const uint32_t nr_keys,
    ...
    struct BloomFilter *const bf =
        (typeof(bf))mempool_alloc(mempool, sizeof(*bf) + bytes);
    bf->bytes = bytes;
    bf->nr_keys = 0;
...
struct BloomTable *bloomtable_build(struct BloomFilter *const *const bfs,
    ...
    struct BloomTable *const bt =
        (typeof(bt))malloc(sizeof(*bt) + (nr_offsets * sizeof(bt->offsets[0])));
    assert(bt);
...
bool bloomtable_dump(struct BloomTable *const bt, FILE *const fo) {
    assert(bt);
...
struct BloomContainer *bloomcontainer_build(struct BloomTable *const bt,
    ...
    for (uint64_t i = 0; i < bt->nr_bf; i++) {
        ...
        const uint64_t alllen_new =
            sizeof(uint16_t) + sizeof(uint16_t) + boxlen_new;
        // switch to next page
        ...
        *pboxid_new = (uint16_t)i;
        uint16_t *const pboxlen_new =
            (typeof(pboxlen_new))(page + off_page + sizeof(*pboxid_new));
        *pboxlen_new = boxlen_new;
        uint8_t *const pbox_new =
            page + off_page + sizeof(*pboxid_new) + sizeof(*pboxlen_new);
    ...
    // write container
    const ssize_t nr_raw_bytes =
        (typeof(nr_raw_bytes))(current_page * BARREL_ALIGN);
    ...
    // alloc new bc
    const uint64_t size_bc =
        sizeof(struct BloomContainer) + (sizeof(uint16_t) * current_page);
...
struct BloomContainer *bloomcontainer_update(struct BloomContainer *const bc,
    ...
    const ssize_t nb0 = pread(bc->raw_fd, old, BARREL_ALIGN,
                              bc->off_raw + (old_page * BARREL_ALIGN));
    ...
    for (uint64_t i = 0; i < bt->nr_bf; i++) {
        ...
        if (i > bc->index_last[old_page]) {
            ...
            const ssize_t nbi = pread(bc->raw_fd, old, BARREL_ALIGN,
                                      bc->off_raw + (old_page * BARREL_ALIGN));
        ...
        const uint16_t *const pboxlen_old =
            (typeof(pboxlen_old))(ptr_old + sizeof(*pboxid_old));
        const uint64_t boxlen_old = (uint64_t)(*pboxlen_old);
        const uint8_t *const pbox_old =
            ptr_old + sizeof(*pboxid_old) + sizeof(*pboxlen_old);

        const uint64_t alllen_old =
            sizeof(*pboxid_old) + sizeof(*pboxlen_old) + boxlen_old;
    ...
    memcpy(bc_new->index_last, index_last,
           sizeof(index_last[0]) * current_page);
    // don't free old bc
    return bc_new;
}
...
bool bloomcontainer_fetch_raw(struct BloomContainer *const bc,
    ...
    for (uint64_t i = 0; i < bc->nr_index; i++) {
        if (bc->index_last[i] >= barrel_id) {
            // fetch page at [i]
            const ssize_t nr = pread(bc->raw_fd, buf, BARREL_ALIGN,
                                     bc->off_raw + (BARREL_ALIGN * i));
...
bool bloomcontainer_dump_meta(struct BloomContainer *const bc, FILE *const fo) {
    ...
    assert(noff == 1);
    const size_t nbar =
        fwrite(&(bc->nr_barrels), sizeof(bc->nr_barrels), 1, fo);
    assert(nbar == 1);
    const size_t nbpb =
        fwrite(&(bc->nr_bf_per_box), sizeof(bc->nr_bf_per_box), 1, fo);
    assert(nbpb == 1);
    const size_t nnri = fwrite(&(bc->nr_index), sizeof(bc->nr_index), 1, fo);
    assert(nnri == 1);
    const size_t nidx =
        fwrite(bc->index_last, sizeof(bc->index_last[0]), bc->nr_index, fo);
...
struct BloomContainer *bloomcontainer_load_meta(FILE *const fi,
    ...
    assert(nbar == 1);
    const size_t nbpb =
        fread(&(bc0.nr_bf_per_box), sizeof(bc0.nr_bf_per_box), 1, fi);
    assert(nbpb == 1);
    const size_t nnri = fread(&(bc0.nr_index), sizeof(bc0.nr_index), 1, fi);
    assert(nnri == 1);
    struct BloomContainer *const bc = (typeof(bc))malloc(
        sizeof(*bc) + (sizeof(bc->index_last[0]) * bc0.nr_index));
    assert(bc);
    bc->raw_fd = raw_fd;
    ...
    const size_t nidx =
        fread(bc->index_last, sizeof(bc->index_last[0]), bc->nr_index, fi);
    assert(nidx == bc->nr_index);
    return bc;
}
