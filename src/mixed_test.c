...
static struct DBParams pstable[] = {
    // tag    vlen  meta_dir       cm_conf_fn     th  pw   gen        range
    // sec   nr
    {"Dummy", 100, "lsmtrie_tmp", "cm_conf1.txt", 1, 100, "uniform",
     UINT64_C(0x100000000000), 3000, 100000},
};
...
static void show_dbparams(const struct DBParams *const ps) {
    printf("MIX_TEST: %s\n", ps->tag);
...
static void mixed_worker(const struct DBParams *const ps) {
    uint64_t keys[100];
    ...
    while (__ts.test_running) {
        ...
        for (uint64_t i = ps->p_writer; i < 100u; i++) {
            ...
            struct KeyValue *const kv = db_lookup(
                __ts.db, sizeof(keys[i]), (const uint8_t *)(&(keys[i])));
...
static void *mixed_thread(void *p) {
...
static void sig_install_all(void) {
    struct sigaction sa;
...
static void mixed_test(const struct DBParams *const p) {
    sig_install_all();
    ...
    for (uint64_t i = 0; i < nth; i++) {
        const int rc =
            pthread_create(&(pth[i]), &attr, mixed_thread, (void *)p);
        assert(rc == 0);
