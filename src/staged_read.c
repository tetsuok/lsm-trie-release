...
static void *read_th(void *const p) {
    (void)p;
    ...
    do {
        for (uint64_t k = 0; k < 100; k++) {
            ...
            struct KeyValue *const kv =
                db_lookup(__ts.db, sizeof(key), (const uint8_t *)(&(key)));
...
static void staged_worker(const struct DBParams *const ps) {
    uint64_t keys[100];
    ...
    for (uint64_t x = 1; __ts.test_running; x++) {
        ...
        do {
            sleep(1);
        } while (db_doing_compaction(__ts.db));
...
static void staged_test(const struct DBParams *const p) {
    ...
    while ((opt = getopt(argc, argv,
        ...
        switch (opt) {
            ...
            case 'c':
                ...
                break;
            case 'a':
                ps.nr_readers = strtoull(optarg, NULL, 10);
                break;
            case 'n':
                ps.nr_cycle = strtoull(optarg, NULL, 10);
                break;
            case 'h': {
                show_dbparams(&ps);
                exit(1);
