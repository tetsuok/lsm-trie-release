...
void multi_level_false_positive_test(void) {
    ...
    printf("multi-level exist keys fp: %" PRIu64 " levels, %" PRIu64
           " keys/level, %" PRIu64 " probes, %" PRIu64 " f-p, %.3lf%%\n",
           nr_levels, nr_keys, level * nr_keys, fp,
           ((double)fp) * 100.0 / ((double)nr_keys));
    fp = 0;
    ...
    printf("multi-level non-exist keys fp: %" PRIu64 " levels, %" PRIu64
           " probes, %" PRIu64 " f-p, %.3lf%%\n",
           nr_levels, level * nr_nonprobe, fp,
           ((double)fp) * 100.0 / ((double)nr_nonprobe));
...
void containertest(void) {
    ...
    // bc
    const int rawfd =
        open("/tmp/bctest", O_CREAT | O_TRUNC | O_RDWR | O_LARGEFILE, 00666);
    assert(rawfd >= 0);
    ...
    printf("match count[0-7]:%" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64
           " %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 " mismatch %" PRIu64
           "\n",
