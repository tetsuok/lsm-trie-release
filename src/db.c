...
static bool compaction_feed(struct Compaction *const comp) {
    ...
    metatable_feed_barrels_to_tables(mt, token, nr_fetch, arena, comp->tables,
                                     compaction_select_table, comp->sub_bit);
    return true;
}
...
static void *thread_compaction_bc(void *const p) {
    ...
    assert(i < 8);
    struct BloomContainer *const new_bc =
        compaction_update_bc(comp->db, comp->mbcs_old[i], comp->tables[i]->bt);
...
static bool db_touch_dir(const char *const root_dir,
...
static struct ContainerMapConf *db_load_cm_conf(const char *const fn) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        cm_conf->hints[i] =
            hint * UINT64_C(1024) * UINT64_C(1024) * UINT64_C(1024);  // *GB
        count++;
    }
